...
fn test_get_client_proof() {
	...
	server_public_key := big.integer_from_radix('', 16) or {
		panic(err)
	}
	client_secret_key := big.integer_from_radix('676add8528d92dd956aab0332a197462', 16) or {
		panic(err)
	}
	expected_scramble := big.integer_from_radix('34239ef7c032bd74949aac7892d68dff22a84049', 16) or {
		panic(err)
	}
	expected_user_hash := big.integer_from_radix('b5397eedf64eacec1be2fb66ffd7009f2381dd5a', 16) or {
		panic(err)
	}
...
fn get_server_session(user string, password string, salt []u8, client_public_key big.Integer, server_public_key big.Integer, server_secret_key big.Integer) []u8 {
	...
	avu := (client_public_key * vu).mod_euclid_checked(prime) or { panic(err) }
...
fn get_server_seed(v big.Integer) (big.Integer, big.Integer) {
	...
	kv := (k * v).mod_euclid_checked(prime) or { panic(err) } // kv = (k * v) % N
	server_public_key := (kv + gb).mod_euclid_checked(prime) or { panic(err) } // B = (kv + gb) % N
...
fn test_secure_remote_password() {
	...
	server_key := get_server_session(user, password, salt, client_public_key, server_public_key,
		server_secret_key)
	_, client_key_one := get_client_proof(user, password, salt, client_public_key,
		server_public_key, client_secret_key, 'Srp')
	client_key_one_len := client_key_one.len
	for i := 0; i < client_key_one_len; i++ {
		assert client_key_one[i] == server_key[i]
	}

	_, client_key_two := get_client_proof(user, password, salt, client_public_key,
		server_public_key, client_secret_key, 'Srp256')
