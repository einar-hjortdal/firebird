...
// The server disconnects when attempting to give a datetime parameter.
...
fn test_statement_time_params() {
	...
	// Looks like Firebird inserts time_tz as time instead
	// println(rows[7])
	// println(v_date_time)
	assert id_string == 3
	assert id_date_time == 8
	assert v_string.Time.hour == 10
	// assert v_date_time.Time.hour == 10
	assert v_string.Time.minute == 34
	// assert v_date_time.Time.minute == 34
	assert v_string.Time.second == 56
	// assert v_date_time.Time.second == 56
	...
	// Looks like Firebird inserts timestamp_tz as timestamp instead
	// println(rows[9])
	assert id_string == 5
